...
function fillBresenhamReta(vertices, canvas) {
...
function fillBresenhamTriangulo(vertices, canvas) {
    const MAX_SIZE = vertices.length;
    let { x_canvas: X0, y_canvas: Y0 } = toCanvas(vertices[0], vertices[1], canvas);
    let { x_canvas: X1, y_canvas: Y1 } = toCanvas(vertices[2], vertices[3], canvas);
    let { x_canvas: X2, y_canvas: Y2 } = toCanvas(vertices[4], vertices[5], canvas);
    let newVertices = [];
    ...
    for (let sides = 0; sides < 3; sides++) {
        ...
        if (Math.abs(m) <= 1) {
            ...
            for (let step = 0; i < MAX_SIZE && step < numSteps; step++) {
                ...
                const { x_web: x_GL, y_web: y_GL } = toWebGL(x, y, canvas);
                newVertices.push(x_GL);
                newVertices.push(y_GL);
                drawnPoints++;
            }
        } else {
            let p = 2 * dx - dy;
